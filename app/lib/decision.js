...
//   - First seen <= 48h   : avian influenza testing must happen within 48 hours
...
const GATING_ANSWERS = ['stillThere', 'seenMoreThan48h', 'reachable', 'condition', 'species', 'count']
...
function decide (data) {
  ...
  if (data.seenMoreThan48h === 'yes') {
    reasons.push('The bird was first seen more than 48 hours ago. Avian influenza testing needs to happen within 48 hours of death, so this bird is no longer suitable for testing.')
    return outcome(false, false, reasons)
  }
...
function explain (data) {
  ...
  const checks = [
    ...
    {
      rule: 'First seen within 48h',
      detail: 'seenMoreThan48h !== "yes"',
      value: data.seenMoreThan48h ? (data.seenMoreThan48h === 'yes' ? '> 48h' : 'within 48h') : '—',
      status: gate(data.seenMoreThan48h, 'yes')
    },
    {
