...
const VERSIONS = [
  {
    ...
    current: true,
    summary: 'Alpha Prototype. Reordered to screener, date seen, location, bird type and number, reachability, condition, then photo, other information, contact and check answers. Adds six contextual exit pages, each shown as soon as its answer rules out collection: sick or injured bird, seen more than 4 days ago, a Northern Ireland location (prototype test toggle), below the collection threshold, cannot be reached safely, or decomposed. A mass mortality (5+ birds) still overrides reachability and condition. The sick, too-old, Northern Ireland, below-threshold and not-reachable exits give no reference number; the decomposed exit gives a REP reference; a collection gives a WSF reference.',
