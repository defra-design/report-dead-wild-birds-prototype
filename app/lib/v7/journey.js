...
const VALIDATORS = {
  ...
  location: function (body, data) {
    ...
    if (method === 'describe' && isBlank(body.locationDescription)) {
      return [{ field: 'locationMethod', message: 'Describe where you saw the bird.' }]
    }
    if ((body.locationDescription || '').trim().length > 500) {
      return [{ field: 'locationMethod', message: 'Description must be 500 characters or less.' }]
    }
  ...
  contact: function (body, data) {
    ...
    if (!email) {
      errors.push({ field: 'email', message: 'Email address must be provided.' })
